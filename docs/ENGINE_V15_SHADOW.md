...
7. `CONFIRMED` when all shadow gates pass and V14 classifies BUY.
8. `WAIT_TRIGGER` when shadow quality passes but V14 has not classified BUY.
...
3. Several complete international runs confirm that thresholds are not
   unintentionally U.S.-specific.
