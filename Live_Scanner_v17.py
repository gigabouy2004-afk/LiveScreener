...
from v17_mtf import (
    ...
    US_MARKET_TZ,
    evaluate_mtf_source,
    is_completed_us_trading_week,
    us_market_context as get_v17_us_market_context,
    validate_us_listing,
)
...
# ==============================================================================
# VERSION 17 SHADOW - COMPLETED D1 BASELINE + COMPLETED 4H/1H DIAGNOSTICS
# ==============================================================================
...
_PERSISTENT_DAILY_CACHE = {
    "enabled": True,
    "directory": os.path.join(tempfile.gettempdir(), "LiveScannerV17", "daily"),
...
def get_market_context(
    ...
    del ticker_symbol, metadata
    resolved = get_v17_us_market_context(now)
    market = "NYSE/NASDAQ"
    market_tz = US_MARKET_TZ
    now_local = pd.Timestamp(resolved["as_at"]).to_pydatetime()
    session_date = now_local.date()
    nominal_start = datetime.combine(
        session_date,
        DEFAULT_SESSION_PROFILE["regular_start"],
        tzinfo=market_tz,
    )
    nominal_end = datetime.combine(
        session_date,
        DEFAULT_SESSION_PROFILE["regular_end"],
        tzinfo=market_tz,
    )
    regular_start_dt = (
        pd.Timestamp(resolved["session_open"]).to_pydatetime()
        if resolved.get("session_open")
        else nominal_start
    )
    regular_end_dt = (
        pd.Timestamp(resolved["session_close"]).to_pydatetime()
        if resolved.get("session_close")
        else nominal_end
    )
    calendar_closed = resolved["phase"] in {"WEEKEND", "HOLIDAY"}
...
def summarize_run_diagnostics(
    ...
    *,
    ...
    return {
        ...
        "v17_development_state_breakup": dict(
            Counter(
                str(
                    row.get("v17_current_development_state")
                    or "UNAVAILABLE"
                )
                for row in successful
            )
        ),
        "v17_true_momentum_candidate_count": sum(
            1
            for row in successful
            if bool(row.get("v17_true_momentum_candidate"))
        ),
        "v17_4h_relation_breakup": dict(
            Counter(
                str(row.get("mtf_4h_relation_to_daily") or "UNAVAILABLE")
                for row in successful
            )
        ),
        "v17_1h_relation_breakup": dict(
            Counter(
                str(row.get("mtf_1h_relation_to_daily") or "UNAVAILABLE")
                for row in successful
            )
        ),
...
def evaluate_prepared_frame(
    ...
    *,
    ...
    result["engine_version"] = "V17_D1_MTF_SHADOW"
    result["v17_shadow_mode"] = True
    result["v17_classification_active"] = False
    result["v17_operational_status_unchanged"] = True
...
def evaluate_stock_momentum(
    ...
    live_candle_mode: str = "completed",
    ...
    requested_mode = (live_candle_mode or "completed").lower()
    if requested_mode != "completed":
        requested_mode = "completed"
    result_template = {
        "ticker": ticker_symbol,
        ...
        "v17_listing_in_scope": None,
        "v17_listing_scope_basis": None,
        "v17_daily_baseline_session_date": None,
        "v17_current_development_state": "INTRADAY_UNAVAILABLE",
    ...
    try:
        ...
        listing_scope = validate_us_listing(ticker_symbol, metadata)
        result_template["v17_listing_in_scope"] = bool(
            listing_scope["in_scope"]
        )
        result_template["v17_listing_scope_basis"] = listing_scope["basis"]
        result_template["v17_listing_exchange_code"] = listing_scope[
            "exchange_code"
        ]
        ...
        def prepare_frame_metadata(
            ...
            weekly_period_complete = is_completed_us_trading_week(
                last_daily_date
            )
        ...
        result["engine_version"] = "V17_D1_MTF_SHADOW"
        result["v17_daily_baseline_session_date"] = result["session_date"]
        mtf_cutoff = (
            pd.Timestamp(as_of_date).tz_localize(US_MARKET_TZ)
            + pd.Timedelta(hours=16)
            if as_of_date
            else datetime.now(US_MARKET_TZ)
        )
        mtf_source = pd.DataFrame()
        mtf_unavailable_reason = None
        if as_of_date is None:
            try:
                mtf_source = execute_yahoo_request(
                    lambda: ticker_data.history(
                        period=V17_INTRADAY_SOURCE_PERIOD,
                        interval="30m",
                        prepost=False,
                        auto_adjust=True,
                    ),
                    operation_name=f"{ticker_symbol} V17 30-minute history",
                    retry_on_empty=True,
                )
            except Exception as mtf_exc:
                mtf_source = pd.DataFrame()
                mtf_unavailable_reason = str(mtf_exc)
        else:
            mtf_unavailable_reason = (
                "Historical 1H/4H evidence requires a supplied replayable "
                "intraday archive; Yahoo current-history requests are not "
                "used for as-of evaluation."
            )
        mtf_output, _ = evaluate_mtf_source(
            mtf_source,
            daily_result=result,
            cutoff=mtf_cutoff,
        )
        result.update(mtf_output)
        result["v17_daily_baseline_session_date"] = result["session_date"]
        result["v17_mtf_unavailable_reason"] = mtf_unavailable_reason
        result["message_details"] = " | ".join(
            part
            for part in (
                str(result.get("message_details") or "").strip(),
                (
                    f"V17DailyBaseline={result['session_date']}; "
                    f"MTFState={result.get('v17_current_development_state')}; "
                    f"4H={result.get('mtf_4h_relation_to_daily')}; "
                    f"1H={result.get('mtf_1h_relation_to_daily')}"
                ),
            )
            if part
        )
...
def scan_watchlist_concurrently(
    ...
    with concurrent.futures.ThreadPoolExecutor(
        ...
        thread_name_prefix="v17-d1-mtf-shadow",
    ) as executor:
...
def prepare_v17_details_output(details_frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if duplicates:
        raise ValueError(
            "V17 output contains duplicate case-insensitive headers: "
            + ", ".join(duplicates)
        )
    return prepared


def format_v17_details_worksheet(details_worksheet) -> None:
    """Freeze through Alpha and format the leading V17 output columns."""
...
if __name__ == "__main__":
    ...
    v17_mtf_suffixes = [
        "available", "timeframe", "bars", "latest_session_date",
        ...
        "stoch_spread_change_1", "stoch_bullish_cross",
        "stoch_bearish_cross", "volume", "volume_per_minute",
        "volume_rate_avg_20_same_slot", "volume_slot_ratio",
        "volume_slot_ratio_change_1", "trend_supportive",
        "trend_check_available", "macd_check_available",
        "adx_check_available", "stoch_check_available",
        "rsi_check_available", "volume_check_available",
        "macd_supportive", "adx_supportive", "stoch_supportive",
        "rsi_supportive", "volume_supportive", "support_count",
        "support_max", "diagnostic_bias", "progression_state",
        "relation_to_daily", "improved_components",
        "regressed_components",
    ]
    v17_columns = [
        "v17_shadow_mode", "v17_classification_active",
        "v17_operational_status_unchanged", "v17_market_scope",
        "v17_listing_in_scope", "v17_listing_scope_basis",
        "v17_listing_exchange_code", "v17_daily_baseline_session_date",
        "v17_daily_baseline_status", "v17_daily_baseline_signal",
        "v17_daily_baseline_momentum_state",
        "v17_daily_primary_regime_passed",
        "v17_current_development_state",
        "v17_true_momentum_candidate", "v17_true_momentum_confirmed",
        "v17_mtf_supportive_count", "v17_mtf_regressing_count",
        "v17_mtf_available_count", "v17_mtf_fresh_count",
        "v17_mtf_fresh_supportive_count",
        "v17_mtf_fresh_regressing_count",
        "v17_intraday_source_interval",
        "v17_intraday_source_rows", "v17_execution_phase",
        "v17_execution_timestamp", "v17_execution_session_date",
        "v17_latest_completed_session_date", "v17_calendar_source",
        "v17_mtf_unavailable_reason",
        *[
            f"{prefix}_{suffix}"
            for prefix in ("mtf_4h", "mtf_1h")
            for suffix in v17_mtf_suffixes
        ],
    ]
    columns_order = [
    ...
    try:
        ...
        df_details_sheet = prepare_v17_details_output(
