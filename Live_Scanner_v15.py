...
# ==============================================================================
# VERSION 15 SHADOW - MOMENTUM QUALITY + ENTRY QUALITY DIAGNOSTICS
# ==============================================================================
...
V15_EMA50_SLOPE_LOOKBACK = 20
V15_EMA200_SLOPE_LOOKBACK = 60
V15_RETURN_LOOKBACKS = (20, 60, 120)
V15_ADX_CONFIRMATION_MIN = 20.0
V15_ADX_RISING_LOOKBACK = 5
V15_BUY_VOLUME_CONFIRMATION_RATIO = 1.0
V15_NEAR_52W_HIGH_MIN_PCT = 85.0
...
DEFAULT_OUTPUT_XLSX = os.path.abspath("D:/TMP/Live_Screener/V15_Shadow_Output.xlsx")
DEFAULT_BACKTEST_CSV = os.path.abspath(
    os.path.join(
        os.path.dirname(__file__),
        "output",
        "V15_Shadow_Backtest.csv",
    )
)
...
def build_v15_shadow_momentum_assessment(
    *,
    ...
    """Score experimental V15 quality evidence without changing classification."""
    checks = {
        ...
        "adx_at_or_above_20": (
            adx_value is not None and adx_value >= V15_ADX_CONFIRMATION_MIN
        ),
        "near_52w_high": (
            price_pct_52w_high is not None
            and price_pct_52w_high >= V15_NEAR_52W_HIGH_MIN_PCT
        ),
        "volume_at_or_above_average": (
            volume_ratio is not None
            and volume_ratio >= V15_BUY_VOLUME_CONFIRMATION_RATIO
        ),
        "entry_extension_safe": (
            ema50_distance_atr is not None
            and ema50_distance_atr <= V15_SAFE_ENTRY_MAX_ATR
        ),
    }
    score = sum(1 for passed in checks.values() if passed)
    if not primary_regime_passed:
        momentum_state = "NONE"
    elif score >= V15_LEADER_MIN_SCORE:
        momentum_state = "LEADER"
    elif score >= V15_DEVELOPING_MIN_SCORE:
        momentum_state = "DEVELOPING"
    else:
        momentum_state = "WEAK"

    return {
        "engine_version": "V15_SHADOW",
        "v15_shadow_mode": True,
        "v15_primary_regime_passed": bool(primary_regime_passed),
...
def apply_v15_shadow_entry_state(result: dict) -> dict:
    """Attach a non-binding V15 entry recommendation to an engine result."""
    updated = dict(result)
    status = str(updated.get("status") or "")
    if status == STATUS_ERROR:
        entry_state = "NOT_APPLICABLE"
        shadow_status = STATUS_ERROR
    elif not updated.get("v15_primary_regime_passed"):
    ...
    updated["v15_classification_active"] = False
    return updated
...
def apply_buy_quality_policies(
    ...
    if updated.get("status") != STATUS_BUY_SIGNAL:
        return apply_v15_shadow_entry_state(updated)
...
def evaluate_frame(df: pd.DataFrame, ticker_symbol: str, config: dict) -> dict:
    def error_result(message: str, output_signal: str) -> dict:
        ...
        return apply_v15_shadow_entry_state({
            "ticker": ticker_symbol,
            "engine_version": "V15_SHADOW",
            "v15_shadow_mode": True,
            "v15_primary_regime_passed": False,
            "momentum_state": "NONE",
    ...
    adx_change_5 = None
    if len(frame["adx"].dropna()) >= V15_ADX_RISING_LOOKBACK + 1:
        adx_change_5 = (
            adx_value
            - float(frame["adx"].dropna().iloc[-V15_ADX_RISING_LOOKBACK - 1])
        )
    ema50_slope_20_pct = percentage_change_over_lookback(
        frame["ema_50"],
        V15_EMA50_SLOPE_LOOKBACK,
    )
    ema200_slope_60_pct = percentage_change_over_lookback(
        frame["ema_200"],
        V15_EMA200_SLOPE_LOOKBACK,
    )
    trailing_returns = {
        lookback: percentage_change_over_lookback(frame["close"], lookback)
        for lookback in V15_RETURN_LOOKBACKS
    }
    ...
    shadow_assessment = build_v15_shadow_momentum_assessment(
        primary_regime_passed=bool(
            macro_trend_ok
            and weekly_trend_ok
    ...
    return apply_v15_shadow_entry_state(metrics)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Version 15 Shadow Momentum Scanner: frozen V14 classifications "
    ...
    enhanced_group = parser.add_argument_group("V15 shadow controls")
    ...
    for run_number, as_of_date in enumerate(as_of_date_list, start=1):
        ...
        print("=" * 80, flush=True)
        print(
            f" STARTING V15 SHADOW MOMENTUM SCANNER | {run_timestamp}",
            flush=True,
        )
        print("=" * 80, flush=True)
    ...
    try:
        ...
        final_lines = [
            "-" * 100,
            "V15 SHADOW MOMENTUM SCANNER - POST EXECUTION SUMMARY",
            "-" * 100,
            ...
            "Classification mode  : V14 frozen; V15 shadow fields are non-binding",
