...
# ==============================================================================
# ENHANCED VERSION 14.2 - BUY-ONLY SUPPLEMENTARY RATINGS AND EVENT RISK
# ==============================================================================
...
DEFAULT_UPCOMING_EARNINGS_WARNING_DAYS = 7
DEFAULT_RECENT_EARNINGS_WARNING_DAYS = 5
...
def rating_category(score: float | None) -> str:
    """Map a normalized -1..1 score to TradingView-style category labels."""
    if score is None:
        return "UNKNOWN"
    if score > 0.5:
        return "STRONG_BUY"
...
def _series_value(series: pd.Series, offset: int = 0) -> float | None:
    if series is None or len(series) <= offset:
        return None
    return _safe_float(series.iloc[-1 - offset])


def _comparison_signal(left: float | None, right: float | None) -> int:
...
def _condition_signal(buy_condition: bool, sell_condition: bool) -> int:
    if buy_condition and not sell_condition:
        return 1
    if sell_condition and not buy_condition:
        return -1
    return 0
...
def _weighted_moving_average(series: pd.Series, window: int) -> pd.Series:
    ...
    return series.rolling(window).apply(
        lambda values: float((values * weights.to_numpy()).sum()) / denominator,
        raw=True,
    )


def calculate_supplementary_technical_rating(df: pd.DataFrame) -> dict:
    """Calculate a transparent TradingView-like daily composite rating.

    This follows TradingView's published 15 moving-average and 11 oscillator
    families. It is independently calculated from the scanner's own OHLCV data
    and is intentionally labelled TV_LIKE rather than TradingView data.
    """
    output = {
        "technical_rating": "UNKNOWN",
        "technical_rating_score": None,
        "technical_ma_rating": "UNKNOWN",
        "technical_ma_score": None,
        "technical_oscillator_rating": "UNKNOWN",
        "technical_oscillator_score": None,
        "technical_buy_signals": 0,
        "technical_neutral_signals": 0,
        "technical_sell_signals": 0,
        "technical_rating_timeframe": "1D",
        "technical_rating_method": "TV_LIKE_15_MA_11_OSC",
        "technical_rating_note": "",
    }
    if df is None or df.empty or len(df) < EMA_SLOW_PERIOD:
        output["technical_rating_note"] = "Insufficient daily history"
        return output
    ...
    try:
        ...
        if not required.issubset(frame.columns):
            output["technical_rating_note"] = "Required OHLCV columns unavailable"
            return output
        ...
        current_price = _series_value(close)
        ma_signals = []
        oscillator_signals = []
        ...
        for window in (10, 20, 30, 50, 100, 200):
            ...
            ma_signals.append(_comparison_signal(current_price, _series_value(sma)))
            ma_signals.append(_comparison_signal(current_price, _series_value(ema)))

        hma_half = _weighted_moving_average(close, 4)
        hma_full = _weighted_moving_average(close, 9)
        hma_9 = _weighted_moving_average((2 * hma_half) - hma_full, 3)
        ma_signals.append(_comparison_signal(current_price, _series_value(hma_9)))

        volume_sum_20 = volume.rolling(20).sum()
        vwma_20 = (close * volume).rolling(20).sum() / volume_sum_20.replace(0, pd.NA)
        ma_signals.append(_comparison_signal(current_price, _series_value(vwma_20)))
        ...
        conversion_now = _series_value(conversion)
        base_now = _series_value(base)
        span_a_now = _series_value(span_a)
        span_b_now = _series_value(span_b)
        ichimoku_buy = all(value is not None for value in (
            current_price,
            conversion_now,
            base_now,
            span_a_now,
            ...
            and base_now > span_a_now
            and conversion_now > base_now
            and current_price > conversion_now
        )
        ichimoku_sell = all(value is not None for value in (
            current_price,
            conversion_now,
            base_now,
            span_a_now,
        ...
        ma_signals.append(_condition_signal(ichimoku_buy, ichimoku_sell))

        rsi = ta.momentum.rsi(close, window=14)
        rsi_now, rsi_prev = _series_value(rsi), _series_value(rsi, 1)
        oscillator_signals.append(_condition_signal(
            bool(rsi_now is not None and rsi_prev is not None and rsi_now < 30 and rsi_now > rsi_prev),
            bool(rsi_now is not None and rsi_prev is not None and rsi_now > 70 and rsi_now < rsi_prev),
        ))

        low_14 = low.rolling(14).min()
        high_14 = high.rolling(14).max()
        raw_stoch = 100 * (close - low_14) / (high_14 - low_14).replace(0, pd.NA)
        stoch_k = raw_stoch.rolling(3).mean()
        stoch_d = stoch_k.rolling(3).mean()
        stoch_k_now, stoch_d_now = _series_value(stoch_k), _series_value(stoch_d)
        oscillator_signals.append(_condition_signal(
            bool(
                stoch_k_now is not None
                and stoch_d_now is not None
                and stoch_k_now < 20
                and stoch_d_now < 20
        ...
        cci_now, cci_prev = _series_value(cci), _series_value(cci, 1)
        oscillator_signals.append(_condition_signal(
            bool(cci_now is not None and cci_prev is not None and cci_now < -100 and cci_now > cci_prev),
            bool(cci_now is not None and cci_prev is not None and cci_now > 100 and cci_now < cci_prev),
        ))
        ...
        adx_now, adx_prev = _series_value(adx), _series_value(adx, 1)
        adx_pos_now, adx_neg_now = _series_value(adx_pos), _series_value(adx_neg)
        oscillator_signals.append(_condition_signal(
            bool(
                adx_now is not None
                and adx_prev is not None
                and adx_pos_now is not None
                and adx_neg_now is not None
        ...
        awesome = ta.momentum.awesome_oscillator(
            high,
            low,
            window1=5,
            window2=34,
        )
        ao_now, ao_prev, ao_two = (
            _series_value(awesome),
            _series_value(awesome, 1),
            _series_value(awesome, 2),
        )
        oscillator_signals.append(_condition_signal(
            bool(
                ao_now is not None
                and ao_prev is not None
                and ao_two is not None
                and (
                    (ao_now > 0 >= ao_prev)
                    or (ao_now > 0 and ao_prev > 0 and ao_now > ao_prev < ao_two)
                )
            ),
            bool(
                ao_now is not None
        ...
        momentum_now, momentum_prev = (
            _series_value(momentum_10),
            _series_value(momentum_10, 1),
        )
        oscillator_signals.append(_comparison_signal(momentum_now, momentum_prev))
        ...
        oscillator_signals.append(_comparison_signal(
            _series_value(macd_indicator.macd()),
            _series_value(macd_indicator.macd_signal()),
        ))
        ...
        srsi_k_now, srsi_d_now = (
            _series_value(stoch_rsi_k),
            _series_value(stoch_rsi_d),
        )
        ema_13 = close.ewm(span=13, adjust=False, min_periods=13).mean()
        uptrend = bool(
            current_price is not None
            and _series_value(ema_13) is not None
            and current_price > _series_value(ema_13)
        )
        oscillator_signals.append(_condition_signal(
            bool(
                not uptrend
                and srsi_k_now is not None
                and srsi_d_now is not None
                and srsi_k_now < 0.2
                and srsi_d_now < 0.2
                and srsi_k_now > srsi_d_now
            ),
            bool(
                uptrend
                and srsi_k_now is not None
        ...
        williams_now, williams_prev = (
            _series_value(williams),
            _series_value(williams, 1),
        )
        oscillator_signals.append(_condition_signal(
            bool(
                williams_now is not None
                and williams_prev is not None
                and williams_now < -80
                and williams_now > williams_prev
            ),
            bool(
                williams_now is not None
                and williams_prev is not None
        ...
        bull_now, bull_prev = _series_value(bull_power), _series_value(bull_power, 1)
        bear_now, bear_prev = _series_value(bear_power), _series_value(bear_power, 1)
        ema_13_now, ema_13_prev = _series_value(ema_13), _series_value(ema_13, 1)
        ema_uptrend = bool(
            ema_13_now is not None
            and ema_13_prev is not None
            and ema_13_now > ema_13_prev
        )
        ema_downtrend = bool(
            ema_13_now is not None
            and ema_13_prev is not None
        ...
        oscillator_signals.append(_condition_signal(
            bool(
                ema_uptrend
                and bear_now is not None
                and bear_prev is not None
                and bear_now < 0
                and bear_now > bear_prev
            ),
            bool(
                ema_downtrend
                and bull_now is not None
                and bull_prev is not None
                and bull_now > 0
                and bull_now < bull_prev
            ),
        ))

        ultimate = ta.momentum.ultimate_oscillator(
            high,
            low,
            close,
            window1=7,
        ...
        ultimate_now = _series_value(ultimate)
        oscillator_signals.append(_condition_signal(
            bool(ultimate_now is not None and ultimate_now > 70),
            bool(ultimate_now is not None and ultimate_now < 30),
        ))

        ma_score = sum(ma_signals) / len(ma_signals)
        oscillator_score = sum(oscillator_signals) / len(oscillator_signals)
        overall_score = (ma_score + oscillator_score) / 2
        all_signals = ma_signals + oscillator_signals

        output.update({
            "technical_rating": rating_category(overall_score),
            "technical_rating_score": round(overall_score, 3),
            "technical_ma_rating": rating_category(ma_score),
            "technical_ma_score": round(ma_score, 3),
            "technical_oscillator_rating": rating_category(oscillator_score),
            "technical_oscillator_score": round(oscillator_score, 3),
            "technical_buy_signals": all_signals.count(1),
            "technical_neutral_signals": all_signals.count(0),
            "technical_sell_signals": all_signals.count(-1),
            "technical_rating_note": (
                "Independent calculation from scanner OHLCV; values may differ "
                "from TradingView because the data feed and current candle differ"
            ),
        })
        return output
    except Exception as exc:
        output["technical_rating_note"] = f"Technical rating unavailable: {exc}"
        return output


def parse_analyst_consensus(
    recommendations,
    ...
    output = {
        ...
        "analyst_rating_source": "Yahoo Finance via yfinance",
        "analyst_rating_note": "",
    }
    try:
        if recommendations is not None and not recommendations.empty:
            current_rows = recommendations[
                recommendations["period"].astype(str).str.lower() == "0m"
            ] if "period" in recommendations.columns else recommendations.iloc[:1]
            row = (
                current_rows.iloc[0]
                if not current_rows.empty
                else recommendations.iloc[0]
            )
            counts = {
                "strongBuy": int(_safe_float(row.get("strongBuy")) or 0),
                "buy": int(_safe_float(row.get("buy")) or 0),
                "hold": int(_safe_float(row.get("hold")) or 0),
                "sell": int(_safe_float(row.get("sell")) or 0),
                "strongSell": int(_safe_float(row.get("strongSell")) or 0),
            }
            ...
            if total:
                ...
                analyst_category = rating_category(score)
                if analyst_category == "NEUTRAL":
                    analyst_category = "HOLD"
                output.update({
                    "analyst_rating": analyst_category,
                    "analyst_rating_score": round(score, 3),
                    "analyst_count": total,
                    "analyst_breakdown": (
                        f"SB={counts['strongBuy']};B={counts['buy']};"
                        f"H={counts['hold']};S={counts['sell']};"
                        f"SS={counts['strongSell']}"
                    ),
                })
        if isinstance(price_targets, dict):
            target_low = _safe_float(price_targets.get("low"))
            target_mean = _safe_float(price_targets.get("mean"))
            target_median = _safe_float(price_targets.get("median"))
            ...
            if (
                ...
                output["analyst_target_upside_pct"] = round(
                    ((target_mean / current_price) - 1) * 100,
                    2,
                )
        if output["analyst_count"] == 0:
            output["analyst_rating_note"] = "No current broker consensus available"
        return output
    except Exception as exc:
        output["analyst_rating_note"] = f"Analyst data unavailable: {exc}"
        return output


def _event_timing(event_timestamp: pd.Timestamp | None) -> str:
    if event_timestamp is None or pd.isna(event_timestamp):
        return "UNKNOWN"
    if (
        event_timestamp.hour == 0
        and event_timestamp.minute == 0
        and event_timestamp.second == 0
    ):
        return "UNKNOWN"
    if event_timestamp.hour < 9 or (
        event_timestamp.hour == 9 and event_timestamp.minute < 30
    ):
        return "BMO"
    if event_timestamp.hour >= 16:
        return "AMC"
    return "MARKET_HOURS"
...
def parse_earnings_event(
    ...
    upcoming_warning_days: int = DEFAULT_UPCOMING_EARNINGS_WARNING_DAYS,
    recent_warning_days: int = DEFAULT_RECENT_EARNINGS_WARNING_DAYS,
) -> dict:
    output = {
        ...
        "earnings_event_risk": "UNKNOWN",
        "earnings_message": "Earnings date unavailable",
        "earnings_data_source": "Yahoo Finance via yfinance",
    }
            ...
            for value in earnings_dates.index:
                ...
                if reference.tzinfo is not None:
                    if timestamp.tzinfo is None:
                        timestamp = timestamp.tz_localize(reference.tzinfo)
                    else:
                        timestamp = timestamp.tz_convert(reference.tzinfo)
    ...
    future_events = [value for value in event_timestamps if value >= reference]
    past_events = [value for value in event_timestamps if value < reference]
    next_event = future_events[0] if future_events else None
    last_event = past_events[-1] if past_events else None

    if next_event is None and isinstance(calendar, dict):
        calendar_dates = calendar.get("Earnings Date")
        if calendar_dates is None:
            calendar_dates = []
        if not isinstance(calendar_dates, (list, tuple)):
            calendar_dates = [calendar_dates]
        for value in calendar_dates:
            try:
                candidate_date = pd.Timestamp(value).date()
            except (TypeError, ValueError):
                continue
            if candidate_date >= reference.date():
                candidate = pd.Timestamp(candidate_date)
                if reference.tzinfo is not None:
                    candidate = candidate.tz_localize(reference.tzinfo)
                next_event = candidate
                break

    if next_event is not None:
        days_to = (next_event.date() - reference.date()).days
        output.update({
            "earnings_next_date": next_event.date().isoformat(),
            "earnings_timing": _event_timing(next_event),
            "days_to_earnings": days_to,
        })

    if last_event is not None:
        days_since = (reference.date() - last_event.date()).days
        output.update({
            "earnings_last_date": last_event.date().isoformat(),
            "days_since_earnings": days_since,
        })

    if (
        output["days_to_earnings"] is not None
        and 0 <= output["days_to_earnings"] <= upcoming_warning_days
    ):
        days_to = output["days_to_earnings"]
        output["earnings_event_risk"] = "UPCOMING"
        output["earnings_message"] = (
            f"Earnings due {'today' if days_to == 0 else f'in {days_to} day(s)'} "
            f"({output['earnings_next_date']} {output['earnings_timing']})"
        )
    elif (
        output["days_since_earnings"] is not None
        and 0 <= output["days_since_earnings"] <= recent_warning_days
    ):
        days_since = output["days_since_earnings"]
        output["earnings_event_risk"] = "RECENT"
        output["earnings_message"] = (
            f"Earnings reported "
            f"{'today' if days_since == 0 else f'{days_since} day(s) ago'} "
            f"({output['earnings_last_date']})"
        )
    elif next_event is not None:
        output["earnings_event_risk"] = "NONE"
        output["earnings_message"] = (
            f"Next earnings in {output['days_to_earnings']} day(s) "
            f"({output['earnings_next_date']} {output['earnings_timing']})"
        )
    elif last_event is not None:
        # A historical report does not prove that no near-term event exists.
        # Without a future date, keep the event risk explicitly unknown.
        output["earnings_event_risk"] = "UNKNOWN"
        output["earnings_message"] = (
            "Upcoming earnings date unavailable; "
            f"last known report was {output['days_since_earnings']} day(s) ago "
            f"({output['earnings_last_date']})"
        )
    return output


def supplementary_field_defaults() -> dict:
    return {
        "supplementary_status": "NOT_APPLICABLE",
        "technical_rating": None,
        "technical_rating_score": None,
        "technical_ma_rating": None,
        "technical_ma_score": None,
        "technical_oscillator_rating": None,
        "technical_oscillator_score": None,
        "technical_buy_signals": None,
        "technical_neutral_signals": None,
        "technical_sell_signals": None,
        "technical_rating_timeframe": None,
        "technical_rating_method": None,
        "technical_rating_note": None,
        "analyst_rating": None,
        "analyst_rating_score": None,
        "analyst_count": None,
        ...
        "analyst_target_high": None,
        "analyst_target_upside_pct": None,
        "analyst_rating_source": None,
        "analyst_rating_note": None,
        "earnings_next_date": None,
        "earnings_timing": None,
        "days_to_earnings": None,
        "earnings_last_date": None,
        "days_since_earnings": None,
        "earnings_event_risk": None,
        "earnings_message": None,
        "earnings_data_source": None,
    }


def enrich_buy_supplementary(
    result: dict,
    frame: pd.DataFrame,
    ticker_data,
    market_context: dict,
    ...
    """Attach non-binding technical, analyst and earnings data to a BUY."""
    enriched = dict(result)
    if enriched.get("status") != STATUS_BUY_SIGNAL:
        return enriched

    technical = calculate_supplementary_technical_rating(frame)
    enriched.update(technical)

    if as_of_date is not None:
        enriched["supplementary_status"] = "TECHNICAL_ONLY_HISTORICAL"
        enriched["analyst_rating"] = "NOT_APPLICABLE_HISTORICAL"
        enriched["earnings_event_risk"] = "NOT_APPLICABLE_HISTORICAL"
        enriched["earnings_message"] = (
            "Current analyst and earnings data are not attached to historical runs"
        )
        return enriched

    recommendations = None
    targets = None
    analyst_fetch_errors = []
    try:
        recommendations = ticker_data.get_recommendations_summary()
    except Exception as exc:
        analyst_fetch_errors.append(f"Consensus={exc}")
    try:
        targets = ticker_data.get_analyst_price_targets()
    except Exception as exc:
        analyst_fetch_errors.append(f"Targets={exc}")
    analyst = parse_analyst_consensus(
        recommendations,
        targets,
        _safe_float(enriched.get("price")),
    )
    if analyst_fetch_errors:
        analyst["analyst_rating_note"] = " | ".join(
            part for part in (
                analyst.get("analyst_rating_note"),
                "; ".join(analyst_fetch_errors),
            )
            if part
        )
    enriched.update(analyst)

    earnings_dates = None
    calendar = None
    try:
        earnings_dates = ticker_data.get_earnings_dates(limit=12)
    except Exception:
        earnings_dates = None
    if earnings_dates is None or bool(getattr(earnings_dates, "empty", True)):
        try:
            calendar = ticker_data.calendar
        except Exception:
            calendar = None
    reference_time = (
        market_context.get("_now_local")
        or datetime.now(market_context.get("_timezone", DEFAULT_MARKET_TZ))
    )
    earnings = parse_earnings_event(
        earnings_dates,
        calendar,
        reference_time,
    )
    enriched.update(earnings)
    enriched["supplementary_status"] = (
        "COMPLETE"
        if (
            enriched.get("technical_rating") != "UNKNOWN"
            and enriched.get("analyst_rating") != "UNKNOWN"
            and enriched.get("earnings_event_risk") != "UNKNOWN"
        )
        else "PARTIAL"
    )

    supplementary_detail = (
        f"SupplementaryTechnical={enriched.get('technical_rating')}"
        f"({enriched.get('technical_rating_score')}); "
        f"Analyst={enriched.get('analyst_rating')}"
        f"({enriched.get('analyst_count') or 0}); "
        f"Earnings={enriched.get('earnings_message')}"
    )
    enriched["message_details"] = " | ".join(
        part for part in (
            str(enriched.get("message_details") or "").strip(),
            supplementary_detail,
        )
        if part
    )
...
DEFAULT_OUTPUT_XLSX = os.path.abspath(
    "D:/TMP/Live_Screener/V14_Enhanced_2_Output.xlsx"
)
DEFAULT_BACKTEST_CSV = os.path.abspath(
    os.path.join(
        os.path.dirname(__file__),
        "output",
        "V14_Enhanced_2_Backtest.csv",
    )
)
...
def evaluate_stock_momentum(
    ...
    try:
        ...
        try:
            result = enrich_buy_supplementary(
                result,
                df,
                ticker_data,
                market_context,
        ...
        except Exception as supplementary_exc:
            # Supplementary data is deliberately non-binding. A provider or
            # calculation failure must never change the engine classification.
            result["supplementary_status"] = (
                "UNAVAILABLE"
                if result.get("status") == STATUS_BUY_SIGNAL
                else "NOT_APPLICABLE"
            )
            if result.get("status") == STATUS_BUY_SIGNAL:
                result["technical_rating"] = (
                    result.get("technical_rating") or "UNKNOWN"
                )
                result["analyst_rating"] = (
                    result.get("analyst_rating") or "UNKNOWN"
                )
                result["earnings_event_risk"] = (
                    result.get("earnings_event_risk") or "UNKNOWN"
                )
                result["earnings_message"] = (
                    result.get("earnings_message")
                    or "Supplementary earnings data unavailable"
                )
                result["technical_rating_note"] = (
                    "Supplementary enrichment unavailable: "
                    f"{supplementary_exc}"
                )
        return result
...
def scan_watchlist_concurrently(
    ...
    with concurrent.futures.ThreadPoolExecutor(
        ...
        thread_name_prefix="v14-enhanced-2",
    ) as executor:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Enhanced Version 14.2 Momentum Scanner: unchanged V14 Enhanced "
            "classification plus BUY-only supplementary technical, analyst, "
    ...
    enhanced_group = parser.add_argument_group("V14 Enhanced 2 controls")
    ...
    for run_number, as_of_date in enumerate(as_of_date_list, start=1):
        ...
        print("=" * 80, flush=True)
        print(
            f" STARTING V14 ENHANCED 2 MOMENTUM SCANNER | {run_timestamp}",
            flush=True,
        )
        print("=" * 80, flush=True)
    ...
    columns_order = [
        ...
        "technical_oscillator_rating", "technical_oscillator_score",
        "technical_buy_signals", "technical_neutral_signals",
        "technical_sell_signals", "technical_rating_timeframe",
        "technical_rating_method", "technical_rating_note",
        "analyst_rating", "analyst_rating_score", "analyst_count",
        "analyst_breakdown", "analyst_target_low", "analyst_target_mean",
        "analyst_target_median", "analyst_target_high",
        "analyst_target_upside_pct", "analyst_rating_source",
        "analyst_rating_note",
        "earnings_next_date", "earnings_timing", "days_to_earnings",
        "earnings_last_date", "days_since_earnings",
        "earnings_event_risk", "earnings_message", "earnings_data_source",
    ...
    try:
        ...
        buy_technical = (
            ",".join(
                f"{row.ticker}({row.technical_rating or 'UNKNOWN'})"
        ...
        buy_analyst = (
            ",".join(
                f"{row.ticker}({row.analyst_rating or 'UNKNOWN'}/"
                f"{int(row.analyst_count or 0)})"
                for row in buy_rows.itertuples()
            )
            if not buy_rows.empty
            else "None"
        )
        earnings_warning_rows = buy_rows[
            buy_rows["earnings_event_risk"].isin(["UPCOMING", "RECENT"])
        ]
        buy_earnings_warnings = (
            "; ".join(
                f"{row.ticker}({row.earnings_message})"
                for row in earnings_warning_rows.itertuples()
            )
            if not earnings_warning_rows.empty
            else "None"
        )
        earnings_unknown_rows = buy_rows[
            buy_rows["earnings_event_risk"].isin(["UNKNOWN", None])
            | buy_rows["earnings_event_risk"].isna()
        ]
        buy_earnings_unknown = (
            ",".join(earnings_unknown_rows["ticker"].astype(str).tolist())
            if not earnings_unknown_rows.empty
            else "None"
        )

        final_lines = [
            "-" * 100,
            "V14 ENHANCED 2 LIVE SCANNER - POST EXECUTION SUMMARY",
            "-" * 100,
            ...
            f"BUY technical rating : {buy_technical}",
            f"BUY analyst consensus: {buy_analyst}",
            f"BUY earnings warnings: {buy_earnings_warnings}",
            f"BUY earnings unknown : {buy_earnings_unknown}",
            ...
            (
                ...
                f"base={config['buy_stoch_max']:.2f}; "
                f"prior-positive Early="
                f"{config['prior_positive_early_stoch_max']:.2f}; "
                f"Continuation="
                f"{config['prior_positive_continuation_stoch_max']:.2f}"
                if config["buy_stoch_max"] > 0
                else "BUY stochastic limit: OFF"
            ),
            "Technical rating      : Independent 1D TV-like 15 MA / 11 oscillator composite",
            "Analyst/earnings data : Yahoo Finance via yfinance; supplementary and non-binding",
